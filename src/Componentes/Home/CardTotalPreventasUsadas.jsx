    ...
    ReporteVenta.getInstance().searchPreventasInServer({
      ...
      // console.log("preventas... respuesta", responseData)
        if (response.data.cantidad > 0 && response.data.preventaReportes) {
          ...
          response.data.preventaReportes.forEach((preventa,ix)=>{
            if(preventa.procesada){
              cantidadx += 1
              totalx += preventa.total
            }
          })
          
          setCantidadVentas(cantidadx);
          setTotal(totalx);
        } else {
          setCantidadVentas(0);
          setTotal(0);
  ...
  return (
    <Card sx={{
      ...
      <CardContent sx={{
        ...
        <RefreshInfoControl
          variableEnSesion={"dashboardRefreshPreventasUsadas"}
        ...
        />
          <div style={{
            ...
            <Typography variant="p" sx={{ display:"block" }}>Usadas</Typography>
